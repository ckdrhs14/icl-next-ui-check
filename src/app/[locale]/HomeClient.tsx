...
const thesisSlides = [
  { src: '/img/main/main_thesis_img1.jpg', text: 'JCRS_Risk factors for normal-tension glaucoma among subgroups of patients' },
  { src: '/img/main/main_thesis_img2.jpg', text: 'JKOS 대한안과학회지' },
  { src: '/img/main/main_thesis_img3.jpg', text: 'JCRS_Risk factors for normal-tension glaucoma among subgroups of patients' },
...
const faqItems = [
  { img: '/img/main/main_faq_img1.jpg', title: 'ICL 수술 후 빛 번짐(링 현상)?\n부작용이 아닌 이유와 적응 기간 총정리', date: '2025-12-01' },
  { img: '/img/main/main_faq_img2.jpg', title: 'ICL 수술 전 검사 궁금증 해결\n산동검사 여부·비용·검사시간 안내', date: '2025-12-01' },
...
const newsData = {
  ...
  images: ['/img/main/main_news_img1.jpg', '/img/main/main_news_img2.jpg', '/img/main/main_news_img3.jpg', '/img/main/main_news_img4.jpg'],
};
...
function useCountUp(target: number, duration = 1500) {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const obs = new IntersectionObserver(([entry]) => { if (entry.isIntersecting) { setStarted(true); obs.disconnect(); } }, { threshold: 0.5 });
    obs.observe(el);
...
export default function HomeClient() {
  ...
  /* count-up refs */
  const countYear = useCountUp(20);
  const countCases = useCountUp(15000);
  const countMin = useCountUp(4);
  const countSec = useCountUp(13);
  const countSat = useCountUp(99);
  ...
  useEffect(() => {
    const SwiperBundle = require('swiper/bundle').default;
    let aboutSw: ReturnType<typeof SwiperBundle> | null = null;
    let memoSw: ReturnType<typeof SwiperBundle> | null = null;
    ...
    function setup() {
      ...
      if (aboutEl) {
        if (isMobile && !aboutSw) {
          aboutSw = new SwiperBundle(aboutEl, { slidesPerView: 1.2, spaceBetween: 12, loop: true, centeredSlides: true, autoplay: { delay: 5000, disableOnInteraction: false } });
        } else if (!isMobile && aboutSw) {
          aboutSw.destroy(true, true); aboutSw = null;
        }
      }
      // Memo
      ...
      if (memoEl) {
        if (isMobile && !memoSw) {
          ...
          memoSw.destroy(true, true); memoSw = null;
        }
      }
    }
    ...
    return () => {
      ...
      try { aboutSw?.destroy(true, true); } catch { /* noop */ }
      try { memoSw?.destroy(true, true); } catch { /* noop */ }
  ...
  useEffect(() => {
    ...
    thesisSwiperRef.current = new SwiperBundle(el, {
      ...
      on: {
        init(sw: SwiperType) { syncTxt(sw.realIndex); },
        slideChange(sw: SwiperType) { syncTxt(sw.realIndex); },
      },
    });
    ...
    return () => {
      if (thesisSwiperRef.current) {
        ...
        try { thesisSwiperRef.current.destroy(true, true); } catch { /* noop */ }
  ...
  return (
    <>
      ...
      <section className={s.heroSection} data-header-bg="white">
        <video src="/video/main_top_video.mp4" autoPlay muted loop playsInline poster="/img/main/main_top_video_poster.jpg" />
      </section>
          ...
          <div className={s.aboutInfoWrap}>
            <figure className={s.aboutTxtWrap}>
              ...
              <div className={s.aboutDesc} data-aos="fade-up">
                <p>같은 수술이라도 결과가 다른 이유는 분명합니다.<br />닥터아이씨엘 이동훈 대표원장은 단순한 &apos;ICL 레퍼런스 닥터&apos;를 넘어, 의사를 가르치는 의사인 <b className={s.highlight}>&apos;ICL 엑스퍼트 인스트럭터(Expert Instructor)&apos;</b>입니다.</p>
                <p>의사 교육을 전담하는 엄격한 기준과 압도적인 숙련도,<br />수술의 정석을 만드는 그 실력 그대로 당신의 소중한 눈을 직접 집도합니다.</p>
              </div>
            </figure>
              ...
              <div className="swiper-wrapper">
                {aboutImages.map((img, i) => (
                  <div key={i} className={`swiper-slide ${s.aboutImgItem}`}>
                    <Image src={img.src} alt={img.alt} width={250} height={330} style={{ width: '100%', height: 'auto' }} />
                  </div>
                ))}
              </div>
            </div>
              ...
              <div key={i} className={s.introImgItem} data-aos="fade-up" data-gsap="intro-img-item">
                <div className={s.introImgItemBox}>
                  <div className={s.introImgItemImg} data-gsap="intro-img">
                    <Image src={img.src} alt={img.alt} width={400} height={700} style={{ width: '100%', height: 'auto' }} />
                  </div>
            ...
            <figure className={s.introLastContent}>
              <div className={s.introLastInner}>
                ...
                <div className={s.introLastImgWrap}>
                  <Image src="/img/main/main_intro_doctor.png" alt="이동훈 원장님" width={530} height={800} style={{ width: '100%', height: 'auto' }} />
                </div>
                <div className={s.introLastTxtWrap}>
                  <p>닥터아이씨엘 이동훈 대표원장은 ICL 레퍼런스 닥터를 거쳐, <br />현재는 <b>전 세계 상위 1% 수준의 의사 교육 전담 포지션인 &apos;ICL 엑스퍼트 인스트럭터&apos;로 활동</b>하고 있습니다.</p>
                  <p>전문의에게 수술을 가르치는 더 엄격하고 정교한 기준. <br />의사를 가르치는 그 최고의 기술력이 닥터아이씨엘에서는 수술의 기본이 됩니다. <br /><b>결과의 차이, 집도의의 급(級)에서 시작</b>됩니다.</p>
                </div>
              </div>
            </figure>
          </div>
      ...
      <section className={s.expertiseSection} data-gsap="expertise">
        <div className={s.expertiseTitWrap}>
          <div className={s.cateTitWrap}>
            ...
            <h3 data-aos="fade-left">같은 수술, 다른 결과 이유는 <span className={s.highlight}>&apos;집도의&apos;에 있습니다.</span></h3>
            <p data-aos="fade-left">수술의 완성도는 장비가 아닌, 집도의의 경험과 판단에서 결정됩니다.<br />수많은 케이스를 통해 축적된 노하우가 결과의 차이를 만들어냅니다.</p>
          </div>
        </div>
          ...
          <div className={s.expertiseRoller} data-gsap="expertise-roller">
                ...
                <div key={i} className={s.expertiseItem} data-gsap="expertise-item">
                  <div className={s.expertiseItemInner}>
                    <div className={s.expertiseItemTxtWrap}>
                      ...
                      <div className={s.expertiseItemDesc} data-aos="fade-left"><p>{item.desc}</p></div>
                    </div>
                    <div className={s.expertiseItemImgWrap}>
                      <div className={s.expertiseItemImg} data-aos="fade-in">
                        <Image src={item.img} alt={item.ko} width={800} height={500} style={{ width: '100%', height: 'auto' }} />
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className={s.expertiseDarkPanel} data-gsap="expertise-dark">
              <div className={s.decoTxtWrap}>
                <span className={`${s.decoTxt} ${s.decoTxt1}`} data-gsap="deco-txt1">ICL Expert Instructor</span>
                <span className={`${s.decoTxt} ${s.decoTxt2}`} data-gsap="deco-txt2">ICL Expert Instructor</span>
              </div>
              <div className={s.expertiseDarkPanelTit}>
                <h3>&ldquo;당신의 집도의는 누구에게<br className={s.moBlock} /> 수술을 배웠습니까?&rdquo;</h3>
      ...
      <section className={`${s.contentSection} ${s.experienceSection}`}>
        <div className={s.innerCon}>
          <div className={`${s.cateTitWrap} ${s.centerTit}`}>
            ...
            <h3 data-aos="fade-up">ICL 엑스퍼트 인스트럭터는 <span className={s.highlight}>경험으로 증명됩니다.</span></h3>
          ...
          <div className={s.experienceNav} data-aos="fade-up">
            {experienceTabs.map((tab, i) => (
              <div key={i} className={`${s.experienceNavItem} ${activeExp === i ? s.experienceNavItemActive : ''}`} onClick={() => { setActiveExp(i); expSwiperRef.current?.slideToLoop(i); }}>
                <div className={s.experienceNavCircleWrap}><span className={s.experienceNavCircle} /></div>
                <h5>{tab}</h5>
              </div>
            ))}
          </div>
          <div className={s.experienceContentWrap}>
            <Swiper data-gsap="experience-swiper" modules={[EffectFade, Autoplay]} effect="fade" fadeEffect={{ crossFade: true }} speed={600} loop={true} autoplay={{ delay: 5000, disableOnInteraction: false }} onSwiper={sw => { expSwiperRef.current = sw; sw.autoplay.stop(); }} onSlideChange={sw => setActiveExp(sw.realIndex)} allowTouchMove={true}>
              {/* Slide 1: Stats */}
              <SwiperSlide>
                <div className={s.experienceSlide}>
                  <div className={s.experienceSlideImg}>
                    <Image src="/img/main/main_experience_img1.jpg" alt="ICL에 집중해 온 이동훈 대표 원장" width={795} height={530} style={{ width: '100%', height: 'auto' }} />
                  </div>
                  <div className={s.experienceSlideInfo}>
                    <div className={s.experienceTitWrap}>
                      <h5><span className={s.highlight}>1 C</span>hoice for <span className={s.highlight}>L</span>ife</h5>
                      <p>ICL에 집중해 온 이동훈 대표 원장</p>
                    </div>
                    <div className={s.statGrid}>
                      <div className={s.statItem} ref={countYear.ref}>
                        <div className={s.statNum}>
                          <span className={s.statCount}>{countYear.count}</span>
                          <span className={s.statUnit}>년</span>
                          <span className={s.statPlus}><svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M12 6.857H6.857V12H5.143V6.857H0V5.143h5.143V0h1.714v5.143H12v1.714Z" fill="black"/></svg></span>
                        </div>
                        <div className={s.statTxt}><h5>ICL에 집중해온 시간</h5></div>
                      </div>
                      <div className={s.statItem} ref={countCases.ref}>
                        <div className={s.statNum}>
                          <span className={s.statCount}>{countCases.count.toLocaleString()}</span>
                          <span className={s.statUnit}>건</span>
                          <span className={s.statPlus}><svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M12 6.857H6.857V12H5.143V6.857H0V5.143h5.143V0h1.714v5.143H12v1.714Z" fill="black"/></svg></span>
                        </div>
                        <div className={s.statTxt}><h5>ICL 수술 집도 건 수</h5></div>
                      </div>
                      <div className={s.statItem} ref={countMin.ref}>
                        <div className={s.statNum}>
                          <span className={s.statCount}>{countMin.count}</span>
                          <span className={s.statUnit}>분</span>
                          <span className={s.statCount}>{countSec.count}</span>
                          <span className={s.statUnit}>초</span>
                        </div>
                        <div className={s.statTxt}>
                          <h5>평균 수술 시간<br />단안기준, 렌즈세팅 및 소독시간 제외</h5>
                          <h6>2022.11~2023.7 평균실적</h6>
                        </div>
                      </div>
                      <div className={s.statItem} ref={countSat.ref}>
                        <div className={s.statNum}>
                          <span className={s.statCount}>{countSat.count}</span>
                          <span className={s.statUnit}>%</span>
                        </div>
                        <div className={s.statTxt}><h5>수술 후 만족도<br />(스페인, 한국, 싱가폴 공동조사)</h5></div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <SwiperSlide>
                <div className={s.experienceSlide}>
                  <div className={s.experienceSlideImg}>
                    <Image src="/img/main/main_experience_img2.jpg" alt="의사 대상 ICL 수술 교육 국내 다수 진행" width={795} height={530} style={{ width: '100%', height: 'auto' }} />
                  </div>
                  <div className={s.experienceSlideInfo}>
              ...
              <SwiperSlide>
                <div className={s.experienceSlide}>
                  <div className={s.experienceSlideImg}>
                    <Image src="/img/main/main_experience_img3.jpg" alt="같은 도수, 다른 결과 0.1mm 노모그램 보정" width={795} height={530} style={{ width: '100%', height: 'auto' }} />
                  </div>
                  <div className={s.experienceSlideInfo}>
                    <div className={s.infoList}>
                      {[
      ...
      <section className={`${s.contentSection} ${s.thesisSection}`} data-header-bg="white">
        <div className={`${s.cateTitWrap} ${s.centerTit}`}>
          ...
          <p data-aos="fade-up">ICL 제1저자 연구논문 한국인 최초 안과학 최고 권위 학술지 Ophthalmology 등재</p>
        </div>
            ...
            <div className="swiper-wrapper">
              {[...thesisSlides, ...thesisSlides, ...thesisSlides].map((slide, i) => (
                <div key={i} className="swiper-slide" data-text={slide.text}>
                  <div className={s.thesisSlide} onClick={() => { setThesisPopup(slide.src); thesisSwiperRef.current?.autoplay?.stop(); }}>
                    <Image src={slide.src} alt={slide.text} width={365} height={500} style={{ width: '100%', height: 'auto' }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className={s.thesisTxtWrap} data-thesis-txt-wrap="">
          {thesisSlides.map((slide, i) => (
            <p key={i} data-thesis-txt={i} style={{ display: 'none' }}>{slide.text}</p>
          ))}
        </div>
      </section>

      {/* Thesis Popup — 항상 렌더, CSS로 표시/숨김 (Swiper DOM 충돌 방지) */}
      <div className={`${s.thesisPopup} ${thesisPopup ? s.thesisPopupVisible : ''}`} onClick={() => { setThesisPopup(null); thesisSwiperRef.current?.autoplay?.start(); }}>
        <div className={s.thesisPopupImg}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          {thesisPopup && <img src={thesisPopup} alt="논문" />}
        </div>
        <button className={s.thesisPopupClose} onClick={() => { setThesisPopup(null); thesisSwiperRef.current?.autoplay?.start(); }}>
          <span /><span />
        </button>
      </div>
          ...
          <figure className={`${s.techInfo} ${s.techInfo1}`} data-gsap="tech-info1">
            <div className={s.technologyInner}>
              <figure className={s.technologyTitWrap}>
                <div className={s.cateTitWrap}>
                  <h5 data-aos="fade-left">ICL Expert Instructor</h5>
                  <h3 data-aos="fade-left">그렇다면 <span className={s.highlight}>0.1mm의 판단은</span><br />어떻게 만들어질까요?</h3>
                  <div className={s.techTxtDesc} data-aos="fade-left"><p>감각이 아니라 첨단 장비를 통한 정확한 측정과 기록으로 판단합니다.</p></div>
                </div>
                <div className={s.technologyImgWrap}>
                  <div className={s.technologyImg} data-aos="fade-up"><Image src="/img/main/main_technology_img1_1.jpg" alt="첨단 장비" width={400} height={300} style={{ width: '100%', height: 'auto' }} /></div>
                  <div className={s.technologyImg} data-aos="fade-up"><Image src="/img/main/main_technology_img1_2.jpg" alt="정확한 측정" width={400} height={300} style={{ width: '100%', height: 'auto' }} /></div>
                </div>
              </figure>
              <figure className={s.technologyThumb} data-gsap="tech-thumb1">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/img/main/main_technology_bg.gif" alt="0.1mm의 판단" data-gsap="tech-thumb1-img" />
              </figure>
            </div>
          </figure>
          <figure className={`${s.techInfo} ${s.techInfo2}`} data-gsap="tech-info2">
            <div className={s.technologyInner}>
              <figure className={s.technologyTitWrap}>
                <div className={s.cateTitWrap}>
                  <h5>ICL Expert Instructor</h5>
                  <h3>이러한 과정으로 <br />0.1mm의 판단은 <span className={s.moBlock}><br /></span>완성됩니다.</h3>
                  <p>(0.1mm 노모그램 보정)</p>
                </div>
                <div className={s.technologyImgWrap} style={{ marginTop: 40 }}>
                  <div className={s.technologyImg}><Image src="/img/main/main_technology_img2_1.jpg" alt="노모그램 보정" width={400} height={300} style={{ width: '100%', height: 'auto' }} /></div>
                  <div className={s.technologyImg}><Image src="/img/main/main_technology_img2_2.jpg" alt="노모그램 보정" width={400} height={300} style={{ width: '100%', height: 'auto' }} /></div>
                </div>
              </figure>
              <figure className={s.technologyThumb}>
                <Image src="/img/main/main_technology_img2_thumb.jpg" alt="노모그램 보정" width={800} height={500} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              </figure>
            </div>
          </figure>
        </div>
      ...
      <section className={`${s.contentSection} ${s.memoSection}`}>
        <div data-gsap="memo" className={s.memoInner}>
          <div className={`${s.cateTitWrap} ${s.centerTit} ${s.memoTitWrap}`}>
            <h5>ICL Expert Instructor</h5>
            <h3>이러한 과정으로 <br /><span className={s.highlight}>0.1mm의 판단은</span> 완성됩니다.</h3>
            <p>수술은 끝나도 기록은 끝나지 않습니다</p>
          </div>
          <div className={`swiper ${s.memoList}`} data-mobile-swiper="memo">
            <div className="swiper-wrapper">
              {[1, 2, 3].map(n => (
                <div key={n} className={`swiper-slide ${s.memoItem} ${s[`memoItem${n}` as keyof typeof s]}`}>
                  <Image src={`/img/main/main_memo_img${n}.png`} alt="수술 기록" width={400} height={550} style={{ width: '100%', height: 'auto' }} />
                </div>
              ))}
            </div>
          </div>
      ...
      <section className={`${s.contentSection} ${s.faqSection}`}>
        <div className={s.innerCon}>
          <div className={`${s.cateTitWrap} ${s.centerTit}`}>
            ...
            <h3 data-aos="fade-up">닥터 ICL 안과 고객님들이 <span className={s.highlight}>궁금해 하시는 내용</span></h3>
            ...
            {faqItems.map((item, i) => (
              <div key={i} className={s.faqItem}>
                ...
                <div className={s.faqItemTxt}>
                  <h5>{item.title.split('\n').map((line, j) => <span key={j}>{line}{j < item.title.split('\n').length - 1 && <br />}</span>)}</h5>
                  <p>{item.date}</p>
                </div>
              </div>
            ))}
        ...
        <div className={s.innerCon}>
          <div className={s.newsContainer} data-aos="fade-up">
            <div className={s.newsTop}>
              <Image src="/img/main/symbol_chosunmedia.png" alt="조선미디어" width={200} height={40} style={{ width: 'auto', height: 'auto' }} />
              <h5>신뢰받는 매체가 선택한 <br />닥터 ICL 안과 의료 소식</h5>
            </div>
            <Swiper data-gsap="news-swiper" modules={[EffectFade, Autoplay]} effect="fade" fadeEffect={{ crossFade: true }} speed={600} loop={true} autoplay={{ delay: 5000, disableOnInteraction: false }} onSwiper={sw => { newsSwiperRef.current = sw; sw.autoplay.stop(); }} onSlideChange={sw => setNewsIndex(sw.realIndex)}>
              {newsData.images.map((img, i) => (
                <SwiperSlide key={i}>
                  <div className={s.newsSlide}>
                    <div className={s.newsSlideImg}>
                      <Image src={img} alt="뉴스" width={595} height={400} style={{ width: '100%', height: 'auto' }} />
                    </div>
                    <div className={s.newsSlideTxt}>
                      ...
                      <div className={s.newsSlideDesc}><p>{newsData.desc} ·····</p></div>
                      <a href={newsData.link} target="_blank" rel="noopener noreferrer" className={s.newsLink}>
            ...
            <div className={s.newsThumbNav}>
              {newsData.images.map((img, i) => (
                <div key={i} className={`${s.newsThumbItem} ${newsIndex === i ? s.newsThumbItemActive : ''}`} onClick={() => newsSwiperRef.current?.slideToLoop(i)}>
                  <Image src={img} alt={`뉴스 ${i + 1}`} width={80} height={80} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </div>
              ))}
            </div>
          </div>
      ...
      <section className={s.locationSection}>
        <div className={s.locationMap}>
          <Image src="/img/main/map.jpg" alt="오시는 길" width={1000} height={900} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        </div>
            ...
            <div className={s.locationTit}>
              ...
              <p data-aos="fade-left">
                <button type="button" className={s.addressCopyBtn} onClick={copyAddress}>
                  서울특별시 강남구 강남대로 390,<br className={s.moBlock} /> 20층(역삼동, 미진프라자)
                  <svg xmlns="http://www.w3.org/2000/svg" width="15" height="17" viewBox="0 0 15 17" fill="none"><path d="M5 13.333a1.667 1.667 0 01-1.667-1.666V1.667C3.333 1.208 3.497.816 3.823.49 4.15.163 4.542 0 5 0h7.5c.458 0 .85.163 1.177.49.326.326.49.718.49 1.177v10c0 .458-.164.85-.49 1.177a1.605 1.605 0 01-1.177.49H5zm0-1.666h7.5V1.667H5v10zM1.667 16.667c-.459 0-.851-.164-1.178-.49A1.605 1.605 0 010 15V3.333h1.667V15h9.166v1.667H1.667z" fill="#1B1B1B"/></svg>
                </button>
              </p>
            </div>
            <div className={s.mapBtnList} data-aos="fade-left">
              <div className={`${s.mapBtn} ${s.naverBtn}`}><a href="https://naver.me/Gt1t61PK" target="_blank" rel="noopener noreferrer"><span><Image src="/img/main/icon_naver_map.png" alt="네이버 지도" width={24} height={24} /></span><span>네이버 지도</span></a></div>
              <div className={`${s.mapBtn} ${s.googleBtn}`}><a href="https://maps.app.goo.gl/TD3nU2nwgVGqjrs37" target="_blank" rel="noopener noreferrer"><span><Image src="/img/main/icon_google_map.png" alt="구글 지도" width={24} height={24} /></span><span>구글 지도</span></a></div>
              <div className={`${s.mapBtn} ${s.kakaoBtn}`}><a href="https://place.map.kakao.com/518228682" target="_blank" rel="noopener noreferrer"><span><Image src="/img/main/icon_kakao_map.png" alt="카카오 지도" width={24} height={24} /></span><span>카카오 지도</span></a></div>
              <div className={`${s.mapBtn} ${s.tmapBtn}`}><a href="https://poi.tmobiweb.com/app/share/position?contents=cGtleT0xMTIyNjE2NDAxJnBvaUlkPTExMjI2MTY0" target="_blank" rel="noopener noreferrer"><span><Image src="/img/main/icon_tmap_map.png" alt="티맵" width={24} height={24} /></span><span>티맵</span></a></div>
            </div>
          </div>
          {/* Kakao */}
          <div className={s.locationTxtItem}>
            <div className={s.locationTit}>
              ...
              <p data-aos="fade-left">카카오톡 아이디 검색에서 <br className={s.moBlock} /> &ldquo;닥터아이씨엘안과의원&rdquo;을 검색하세요.</p>
            </div>
          </div>
          {/* Hours + Phone */}
          <div className={`${s.locationTxtItem} ${s.locationFlexItem}`}>
            <div>
              <div className={s.locationTit}><h3 data-aos="fade-left">진료시간</h3></div>
              <ul className={s.locationHoursList} data-aos="fade-left">
                <li><b>월, 화, 목, 금</b><span>AM 09:00 ~ PM 06:00</span></li>
                <li><b>토요일</b><span>AM 08:30 ~ PM 04:00</span></li>
                <li><b>점심시간</b><span>PM 01:00 ~ PM 02:00</span></li>
              </ul>
              <p className={s.locationHoursNote} data-aos="fade-left">* 수요일, 일요일 및 공휴일은 휴무입니다.</p>
            </div>
            <div>
              <div className={s.locationTit} data-aos="fade-left"><h3>상담예약</h3></div>
              <div className={s.locationPhone} data-aos="fade-left"><a href="tel:02-6956-8882">02. 6956. 8882</a></div>
              <div className={s.locationSocialList} data-aos="fade-left">
                {[
                  { href: 'https://www.youtube.com/@driclno.1', img: 'icon_youtube', gray: 'icon_youtube_gray' },
                  { href: 'https://pf.kakao.com/_rXGTG', img: 'icon_kakao', gray: 'icon_kakao_gray' },
                  { href: 'https://www.instagram.com/doctoricl/', img: 'icon_insta', gray: 'icon_insta_gray' },
                  { href: 'https://blog.naver.com/doctor_icl', img: 'icon_blog', gray: 'icon_blog_gray' },
                ].map((social, i) => (
                  <div key={i} className={s.socialBtn}>
                    <a href={social.href} target="_blank" rel="noopener noreferrer">
                      <Image src={`/img/main/${social.gray}.png`} alt="" width={30} height={30} />
                      <Image src={`/img/main/${social.img}.png`} alt="" width={30} height={30} className={s.socialBtnHoverImg} />
                    </a>
                  </div>
                ))}
              </div>
